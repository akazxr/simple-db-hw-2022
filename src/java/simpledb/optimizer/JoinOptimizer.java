...
public class JoinOptimizer {
    ...
    public List<LogicalJoinNode> orderJoins(Map<String, TableStats> stats, Map<String, Double> filterSelectivities,
        ...
        for (int i = 1; i <= joins.size(); i++) {
            ...
            for (Set<LogicalJoinNode> subSet : set) {
                double bestCostSoFar = Double.MAX_VALUE;
                for (LogicalJoinNode j : subSet) {
                    CostCard costCard = computeCostAndCardOfSubplan(stats, filterSelectivities, j, subSet,
    ...
    private CostCard computeCostAndCardOfSubplan(Map<String, TableStats> stats, Map<String, Double> filterSelectivities,
        LogicalJoinNode joinToRemove, Set<LogicalJoinNode> joinSet, double bestCostSoFar, PlanCache pc)
        throws ParsingException {
        ...
        for (LogicalJoinNode j : js) {
            ...
            if (n == null) { // never seen this table before
                n = new DefaultMutableTreeNode(
                    j.t1Alias + " (Cost = " + stats.get(table1Name).estimateScanCost() + ", card = " + stats.get(
                        table1Name).estimateTableCardinality(selectivities.get(j.t1Alias)) + ")");
                root.add(n);
            } else {
            ...
            if (n == null) { // never seen this table before
                ...
                n = new DefaultMutableTreeNode(j.t2Alias == null
                    ? "Subplan"
                    : (j.t2Alias + " (Cost = " + stats.get(table2Name).estimateScanCost() + ", card = " + stats.get(
                        table2Name).estimateTableCardinality(selectivities.get(j.t2Alias)) + ")"));
                root.add(n);
            } else {
